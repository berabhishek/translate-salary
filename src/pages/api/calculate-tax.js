...
export const internals = {
  ...
  async getTaxBrackets(countryCode) {
...
export default async function handler(req, res) {
  ...
  let body;
  try {
    body = internals.parseRequestBody(req);
    const { sourceCode, salary } = body;
    internals.validateParams({ sourceCode, salary });
    const brackets = await internals.getTaxBrackets(sourceCode);
    if (!Array.isArray(brackets) || brackets.length === 0) {
      return res.status(404).json({ error: 'No tax brackets found for country' });
    }
    const income = Number(salary);
    const { tax, debug } = internals.calculateProgressiveTax(income, brackets);
